...
const FixedEquipmentQuoteTool = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    equipment: [
      {
        ...
        type: '',
        specifications: '',
        ownership: 'rent', // 'rent' or 'lease' only
        rentalMonthly: 0,
    ...
    staff: [
      { role: 'Tech', salary: 60000 },
      { role: 'Med Clerk', salary: 60000 }
    ],
    
    // Fixed Building Expenses
    buildingCostPerSqFt: 25,
    buildingSquareFeet: 1500,
    utilitiesMonthly: 800, // electric, water, internet combined
    insuranceAnnual: 18000,
    marketingAnnual: 25000,
    housekeepingMonthly: 500,
    billingServicePercent: 6,
    trackingAnnual: 3000,
    physicistAnnual: 6000,
    itAnnual: 12000,
    
    // Additional costs
    medicalSuppliesAnnual: 15000,
  ...
  const addStaff = () => {
    if (formData.staff.length < 5) {
      setFormData(prev => ({
        ...prev,
        staff: [...prev.staff, { role: 'Additional Staff', salary: 60000 }]
      }));
    }
  };
  ...
  const calculateQuote = () => {
    ...
    const calculateYearExpenses = (yearIndex, revenue) => {
      ...
      const buildingCost = buildingCostAnnual * (1 + yearIndex * 0.03); // 3% annual increase
      const utilities = formData.utilitiesMonthly * 12 * (1 + yearIndex * 0.03);
      const housekeeping = formData.housekeepingMonthly * 12 * (1 + yearIndex * 0.02);
      const insurance = formData.insuranceAnnual * (1 + yearIndex * 0.03);
      const marketing = formData.marketingAnnual;
      const billing = revenue * (formData.billingServicePercent / 100);
      const tracking = formData.trackingAnnual * (1 + yearIndex * 0.02);
    ...
    const fixedCosts = expensesByYear[0].staffing + expensesByYear[0].serviceContract + 
                      expensesByYear[0].equipmentPayments + expensesByYear[0].buildingCost +
                      expensesByYear[0].utilities + expensesByYear[0].housekeeping +
    ...
    const quoteNumber = `FE-${Date.now().toString().slice(-8)}`;
                  ...
                  <div key={equip.id} className="mb-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <div className="mb-4">
                      ...
                      <input type="text" value={equip.type} 
                        ...
                        placeholder="e.g., MRI, CT Scanner, X-Ray" />
                    </div>
                    ...
                    {/* Ownership Options - Only Rent or Lease */}
                    <div className="mb-4">
                          ...
                          <div className="bg-white p-3 rounded-lg border border-purple-300">
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-xl font-bold text-purple-600">
                                  ${(calculateLeasePayment(equip.leaseAmount, equip.leaseTerm, equip.leaseRate) * equip.leaseTerm).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                                </p>
                              </div>
                            </div>
                          </div>
        ...
        {step === 3 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    ...
                    <input type="number" name="buildingCostPerSqFt" value={formData.buildingCostPerSqFt} onChange={handleInputChange}
                  ...
                  <div>
                    ...
                    <input type="number" name="buildingSquareFeet" value={formData.buildingSquareFeet} onChange={handleInputChange}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#03989e] focus:border-[#03989e]" />
                  </div>
                  <div className="bg-[#03989e]/10 p-3 rounded-lg border border-[#03989e]/30 flex items-center justify-center">
                    <div className="text-center">
                      <p className="text-xs text-gray-600">Annual Building Cost</p>
                      <p className="text-xl font-bold text-[#03989e]">${(formData.buildingCostPerSqFt * formData.buildingSquareFeet).toLocaleString()}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <input type="text" value={formatNumber(formData.utilitiesMonthly)}
                      onChange={(e) => setFormData(prev => ({...prev, utilitiesMonthly: parseFormattedNumber(e.target.value)}))}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff66c4] focus:border-[#ff66c4]" />
                    <p className="text-xs text-gray-500 mt-1">Electric, Water, Internet</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Insurance ($/year)</label>
                    <input type="text" value={formatNumber(formData.insuranceAnnual)}
                      onChange={(e) => setFormData(prev => ({...prev, insuranceAnnual: parseFormattedNumber(e.target.value)}))}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff66c4] focus:border-[#ff66c4]" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Marketing ($/year)</label>
                    <input type="text" value={formatNumber(formData.marketingAnnual)}
                      onChange={(e) => setFormData(prev => ({...prev, marketingAnnual: parseFormattedNumber(e.target.value)}))}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff66c4] focus:border-[#ff66c4]" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Housekeeping ($/month)</label>
                    <input type="text" value={formatNumber(formData.housekeepingMonthly)}
                      onChange={(e) => setFormData(prev => ({...prev, housekeepingMonthly: parseFormattedNumber(e.target.value)}))}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff66c4] focus:border-[#ff66c4]" />
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-800 uppercase tracking-wide mb-3">Professional Services & Operations</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Tracking ($/year)</label>
                    <input type="text" value={formatNumber(formData.trackingAnnual)}
                      onChange={(e) => setFormData(prev => ({...prev, trackingAnnual: parseFormattedNumber(e.target.value)}))}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff66c4] focus:border-[#ff66c4]" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Physicist ($/year)</label>
                    <input type="text" value={formatNumber(formData.physicistAnnual)}
                      onChange={(e) => setFormData(prev => ({...prev, physicistAnnual: parseFormattedNumber(e.target.value)}))}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff66c4] focus:border-[#ff66c4]" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">IT ($/year)</label>
                    <input type="text" value={formatNumber(formData.itAnnual)}
                      onChange={(e) => setFormData(prev => ({...prev, itAnnual: parseFormattedNumber(e.target.value)}))}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#ff66c4] focus:border-[#ff66c4]" />
                  </div>
                </div>
              </div>
                        ...
                        {formData.equipment.map((equip, idx) => (
                          <tr key={idx}>
                            ...
                            <td className="px-3 py-2 border border-gray-300 text-right text-gray-700 text-sm">
                              {equip.ownership === 'rent' ? 'N/A' : `$${formatNumber(equip.leaseAmount)}`}
                            </td>
                      ...
                      <tbody>
                        {quote.revenueByYear.map((rev, idx) => (
                          <tr key={idx}>
                            ...
                            <td className={`px-2 sm:px-3 py-2 border border-gray-300 text-right font-semibold ${quote.profitByYear[idx] >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              ${Math.round(quote.profitByYear[idx]).toLocaleString()}
                            </td>
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between border-t border-green-200 pt-2">
                      ...
                      <span className={`font-bold ${quote.cumulativeProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        ${Math.round(quote.cumulativeProfit).toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={`font-semibold ${quote.dailyProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        ${Math.round(quote.dailyProfit).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={`font-semibold ${quote.monthlyProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        ${Math.round(quote.monthlyProfit).toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={`font-semibold ${quote.profitPerScan >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        ${quote.profitPerScan.toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
...
export default FixedEquipmentQuoteTool;
